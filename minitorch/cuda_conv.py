    ...
    for batch_idx in range(batch_size):
        ...
        for channel_start in range(0, input_channels, BLOCK_DIM):
            ...
            for kernel_start in range(0, kernel_width, BLOCK_DIM):
                ...
                if channel_cache_idx < input_channels and kernel_idx < kernel_width:
                    cache_idx = (
                        current_channel * b_batch_stride
                        + channel_cache_idx * b_channel_stride
                        + kernel_idx * b_width_stride
                    )
                ...
                for width_offset in range(0, CACHE_DIM, BLOCK_DIM):
                    ...
                    if channel_cache_idx < input_channels and 0 <= pos < input_width:
                        input_cache_idx = (
                            ...
                            + channel_cache_idx * a_channel_stride
                            + pos * a_width_stride
                        )
                        input_cache[thread_x, width_offset + thread_y] = a_storage[
                            input_cache_idx
                        ]
                    else:
                ...
                if thread_y == 0 and current_width < output_width:
                    for channel_idx_inner in range(
                        channel_start, min(input_channels, channel_start + BLOCK_DIM)
                    ):
                        for kernel_idx_inner in range(
                            ...
                            if is_reverse:
                                min_bound = (
                                    tile_start_width - kernel_start - BLOCK_DIM + 1
                                )
                            else:
        ...
        if thread_y == 0 and current_width < output_width:
            output_idx = (
                ...
                + current_channel * out_channel_stride
                + current_width * out_width_stride
            )
...
class Conv1dFun(Function):
    ...
    def forward_inner(
        ...
        cuda_conv1d[blockspergrid, threadsperblock](
...
def _cuda_conv2d(
    ...
    * Only write to global memory once per kernel
    ...
    assert (
        ...
        and input_channels == kernel_in_channels
        and output_channels == kernel_out_channels
    ), "Shape mismatch"
    assert (
        output_width <= input_width and output_height <= input_height
    ), "Output dims exceed input dims."
    ...
    for batch_i in range(batch_size):
        out_pos = (
            batch_i * out_batch_stride
            + current_out_channel * out_channel_stride
            + current_height * out_height_stride
            + current_width * out_width_stride
        )
        tmp = 0.0
            ...
            for kh_start in range(0, kernel_height, BLOCK_DIM):
                for kw_start in range(0, kernel_width, BLOCK_DIM):
                    ...
                    if kh_now < kernel_height and kw_now < kernel_width:
                        weight_cache_pos = (
                            current_out_channel * b_batch_stride
                            + in_channel_i * b_channel_stride
                            + kh_now * b_height_stride
                            + kw_now * b_width_stride
                        )
                    ...
                    for w_cache_bias in range(0, CACHE_DIM, BLOCK_DIM):
                        for h_cache_bias in range(0, CACHE_DIM, BLOCK_DIM):
                            ...
                            if (
                                0 <= w_cache_pos < input_width
                                and 0 <= h_cache_pos < input_height
                            ):
                                input_cache_pos = (
                                    batch_i * a_batch_stride
                                    + in_channel_i * a_channel_stride
                                    + h_cache_pos * a_height_stride
                                    + w_cache_pos * a_width_stride
                                )
                                input_cache[
                                    (w_cache_bias + thread_x, h_cache_bias + thread_y)
                                ] = a_storage[input_cache_pos]
                            else:
                                input_cache[
                                    (w_cache_bias + thread_x, h_cache_bias + thread_y)
                                ] = 0.0
                    ...
                    if current_height < output_height and current_width < output_width:
                        for khi in range(
                            ...
                            height_cache_min = (
                                tile_start_height - kh_start - BLOCK_DIM + 1
                                if is_reverse
                                else tile_start_height + kh_start
                            )
                            height_cache_max = height_cache_min + CACHE_DIM

                            if not (
                                0 <= h_now < input_height
                            ...
                            for kwi in range(
                                ...
                                width_cache_min = (
                                    tile_start_width - kw_start - BLOCK_DIM + 1
                                    if is_reverse
                                    else tile_start_width + kw_start
                                )
                                ...
                                tmp += (
                                    kernel_cache[kwi - kw_start, khi - kh_start]
                                    * input_cache[
                                        (
...
class Conv2dFun(Function):
    ...
    def forward_inner(
        ...
        assert in_channels == in_channels2, "Channels do not match"
        ...
        cuda_conv2d[blockspergrid, threadsperblock](
            *output.tuple(), output.size, *input.tuple(), *weight.tuple(), reversed
        )
